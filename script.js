...
/*$(function() {
    function setColors() {
        $("#box").css({
            "background-color" : "rgba(" + $(".slider").eq(0).val() + "," + $(".slider").eq(1).val() + "," + $(".slider").eq(2).val() + "," + $(".slider").eq(3).val() + ")",
            "backdrop-filter" : "blur(" + $(".slider").eq(4).val() + "px)"
        });
    }

    $(".slider").on('input', function() {
        let spanId = $(this).attr('data-spanid');
